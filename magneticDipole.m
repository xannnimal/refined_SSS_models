...
for i=(1:size(chanpos,1))
    if ch_types(i)==1 %magnetometer calculations
        ...
        magneticFlux(i,:) =dot(BField(i,:),sensing_dir(i,:)); %no integral for point mags
        ...
        flux_plus=dot(BField_plus(i,:),sensing_dir(i,:));
        flux_min=dot(BField_min(i,:),sensing_dir(i,:));
        magneticFlux(i,:)=(flux_plus-flux_min)/0.0168;
    end
end
