...
function mag_flux = grad(chanpos_i,EX_i,EY_i,EZ_i,dip_pos, Q,weights_grad, D_grad)
    ...
    for i=1:8
        ...
        r_hat = norm(r_vec);
        a_vec=bsxfun(@minus, r_vec, dip_pos); %r(i,:)-dip_pos;
        a=norm(a_vec); %magnitude
        r=norm(r_vec);
        F = a*(r*a+r^2-dot(dip_pos,r_vec));
        del_F=((1/r)*a^2+(1/a)*dot(a_vec,r_vec)+2*a+2*r)*r_vec - (a+2*r+(1/a)*dot(a_vec,r_vec)*dip_pos);
        Bfield= (1/(F^2))*(cross(F*Q,dip_pos)-(dot(cross(Q,dip_pos),r_vec)*del_F));
        %Bfield2=Q/r_hat^3;
        B(:,i)= scale*(Bfield);
    end
