function [opm_matrix_avg,R_hat,theta_hat,phi_hat,ch_types] = gen_opm_geometry_avg(filename)
...
opm_avg_len = length(opm_matrix)/4;
opm_matrix_avg = zeros(opm_avg_len,3);

for j=1:opm_avg_len
    if j==1
        opm_matrix_avg(j,:) = mean(opm_matrix(j:4*j,:));
    else
        opm_matrix_avg(j,:) = mean(opm_matrix(4*j-3:4*j,:));
    end
end
