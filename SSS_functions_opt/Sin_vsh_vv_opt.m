...
%
function [Sin,SNin] = Sin_vsh_vv_opt(r_sphere,R,EX,EY,EZ,ch_types,Lin)
...
for ch = 1:nchan
  %disp(ch)
   ...
   for l = 1:Lin
      m = -l:l;
      Sin(ch,l^2:length(m)+l^2 - 1) =  -mu0*vsh_response_opt(R(:,ch),EX(:,ch),EY(:,ch),EZ(:,ch),D,weights,l,m);
      % for m = -l:l
	  %    Sin(ch,count) = -mu0*vsh_response_opt(R(:,ch),EX(:,ch),EY(:,ch),EZ(:,ch),D,weights,l,m);
	  %    count = count + 1;
      % end
   end
end
...
function Sin_element = vsh_response_opt(r,ex,ey,ez,D,weights,l,m)
...
for j = 1:length(weights)
  ...
  cost = cos(theta);
  cosp = cos(phi);
  vs = vsh_modified_in_opt(theta,phi,l,m)'/rn^(l+2);
